...
@Component({
  ...
  imports: [
    TraducirPipe, LegalDocumentoComponent
  ],
  template: `
    <app-legal-documento
      ...
      <p>
        {{ 'Una reserva confirmada obliga al comercio a prestar el servicio en las condiciones anunciadas. Las cancelaciones se rigen por la política que el propio comercio declara en su ficha (flexible, moderada o estricta), que se muestra al cliente antes de pagar.' | t }}
      </p>
      <p>
        {{ 'Las cancelaciones repetidas por parte del comercio afectan a su posición en los resultados y pueden motivar la suspensión de la cuenta.' | t }}
      </p>

      <h2>{{ '5. Precios, comisión y liquidaciones' | t }}</h2>
      <p>
        Los precios los fija el comercio e incluyen los impuestos aplicables. {{ r.marca }} cobra
        al cliente en el momento de la reserva y aplica una <strong>{{ 'comisión' | t }}</strong> {{ 'sobre el importe, según el porcentaje vigente para su categoría o el pactado con el comercio.' | t }}
      </p>
      <p>
        {{ 'La liquidación se abona en la cuenta bancaria declarada por el comercio, descontando la comisión de la plataforma y los costes de la pasarela de pago. El detalle de cada liquidación está disponible en el panel.' | t }}
      </p>

      <h2>{{ '6. Obligaciones del comercio' | t }}</h2>
      <ul>
        <li>{{ 'Prestar el servicio con la diligencia profesional exigible a su actividad.' | t }}</li>
        <li>{{ 'Mantener vigentes los permisos, licencias y seguros que su actividad requiera.' | t }}</li>
        <li>{{ 'Tratar los datos de los clientes conforme a la normativa de protección de datos y usarlos únicamente para prestar el servicio reservado.' | t }}</li>
        <li>{{ 'Atender las reservas recibidas y mantener la disponibilidad publicada.' | t }}</li>
        <li>{{ 'No dirigir a los clientes captados en la plataforma fuera de ella para eludir la comisión.' | t }}</li>
      </ul>
      ...
      <h2>{{ '8. Suspensión y baja' | t }}</h2>
      <p>
        {{ 'El comercio puede darse de baja cuando quiera desde' | t }} <em>{{ 'Panel → Mi cuenta' | t }}</em>{{ ', sin permanencia. Las reservas ya confirmadas deben atenderse o cancelarse conforme a la política declarada.' | t }}
      </p>
      <p>
      ...
      <h2>{{ '10. Cambios en estas condiciones' | t }}</h2>
      <p>
