...
describe('PerfilAlphaComponent', () => {
  ...
  const montar = async (
    niveles: jest.Mock,
    miEstado: jest.Mock,
    ventajas: jest.Mock = jest.fn().mockResolvedValue([]),
  ): Promise<void> => {
    TestBed.resetTestingModule();
    alphaService = { niveles, miEstado, ventajas };
  ...
  const crear = (miEstado: AlphaEstadoApi | null, ventajas: AlphaVentajaApi[] = []): Promise<void> =>
    montar(
      jest.fn().mockResolvedValue(NIVELES),
      jest.fn().mockResolvedValue(miEstado),
      jest.fn().mockResolvedValue(ventajas),
    );
