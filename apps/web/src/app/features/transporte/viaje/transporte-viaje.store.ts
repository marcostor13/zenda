...
import {
  ConfirmacionEntrega, EquipajeTransporte, EspecieMascota, FranjaTransporte, MascotaViaje, ModalidadTransporte,
  ModoHorarioTransporte, NecesidadTransporte, OrdenTransporte, PatronRecurrenciaTransporte, PersonaContactoViaje,
  PreferenciaTransporte, PuntoViaje, SolicitudTransporte, TamanoPerro, TipoServicioTransporte, VueltaTransporte,
  diasDelPatron, hoyEnZona,
} from 'shared';
...
export interface BorradorViaje {
  ...
  filtroIncluidos: PreferenciaTransporte[];
...
export function borradorInicial(): BorradorViaje {
  return {
    ...
    personas: 1,
    equipaje: EquipajeTransporte.SIN_EQUIPAJE,
    preferencias: [],
...
export class TransporteViajeStore {
  ...
  readonly solicitud = computed<SolicitudTransporte | null>(() => {
    const b = this.estado();
    ...
    return {
      ...
      necesidadOtra: b.necesidades.includes(NecesidadTransporte.OTRA) ? b.necesidadOtra || undefined : undefined,
  ...
  elegirTipo(tipo: TipoServicioTransporte): void {
    const cambios: Partial<BorradorViaje> = { tipoServicio: tipo };
    if (tipo === TipoServicioTransporte.URGENTE) {
      cambios.modoHorario = ModoHorarioTransporte.LO_ANTES_POSIBLE;
      cambios.fecha = hoyEnZona();
    }
    if (tipo === TipoServicioTransporte.VIAJO_CON_MASCOTA) cambios.modalidad = ModalidadTransporte.CON_PROPIETARIO;
    if (tipo !== TipoServicioTransporte.VIAJO_CON_MASCOTA && this.estado().modalidad === ModalidadTransporte.CON_PROPIETARIO) {
   ...
   */
  cargarSolicitud(s: SolicitudTransporte, presupuestoId: string, eleccion: EleccionTransporte): void {
  ...
  private horarioCompleto(b: BorradorViaje): boolean {
    ...
    if (b.tipoServicio === TipoServicioTransporte.RECURRENTE && !b.hasta) return false;
    if (b.tipoServicio === TipoServicioTransporte.IDA_VUELTA && b.vueltaModo === VueltaTransporte.OTRO_DIA && !b.vueltaFecha) {
      return false;
    }
    return true;
  }

  private vueltaDe(b: BorradorViaje): SolicitudTransporte['vuelta'] {
    if (b.tipoServicio !== TipoServicioTransporte.IDA_VUELTA) return undefined;
    switch (b.vueltaModo) {
  ...
  private recurrenciaDe(b: BorradorViaje): SolicitudTransporte['recurrencia'] {
    if (b.tipoServicio !== TipoServicioTransporte.RECURRENTE || !b.hasta) return undefined;
    return {
      patron: b.patron,
