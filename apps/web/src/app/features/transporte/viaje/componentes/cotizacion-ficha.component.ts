...
import {
  BusquedaTransportesRespuesta, MODALIDAD_TRANSPORTE_LABELS, ModalidadTransporte, PREFERENCIA_TRANSPORTE_LABELS,
  PreferenciaTransporte, resumenSolicitudTransporte,
} from 'shared';
...
import { ICONO_INCLUIDO } from '../transporte-viaje.opciones';
  ...
  async pedirPresupuesto(): Promise<void> {
    ...
    try {
      ...
      await this.api.pedirPresupuesto({
        servicioIds: [this.servicioId()], solicitud: conModalidad, resumen: resumenSolicitudTransporte(conModalidad),
      });
