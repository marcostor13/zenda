...
import {
  ESPECIE_MASCOTA_LABELS, EquipajeTransporte, EspecieMascota, MAX_MASCOTAS_SOLICITUD, ModalidadTransporte,
  NecesidadTransporte, PreferenciaTransporte, TamanoPerro, calcularTamanoPorPeso, nombreTamanoPerro, normalizarEspecie,
} from 'shared';
...
export class MascotaViajeComponent implements OnInit {
  ...
  readonly form = new FormGroup({
    ...
    equipaje: new FormControl<string>(EquipajeTransporte.SIN_EQUIPAJE, { nonNullable: true }),
  ...
  descripcionPerro(p: PerroApi): string {
    const especie = ESPECIE_MASCOTA_LABELS[normalizarEspecie(p.especie)];
  ...
  private aMascota(p: PerroApi): MascotaElegida {
    return {
      perroId: p._id, nombre: p.nombre, especie: normalizarEspecie(p.especie), tamano: tamanoDe(p),
  ...
  private precargarNecesidades(p: PerroApi): void {
    ...
    if (p.requiereTransportin) actuales.add(NecesidadTransporte.TRANSPORTIN);
    if (p.medicacion?.length) actuales.add(NecesidadTransporte.MEDICACION);
  ...
  private volcarEnStore(): void {
    ...
    this.store.actualizar({
      ...
      equipaje: v.equipaje as EquipajeTransporte,
      necesidades: v.necesidades as NecesidadTransporte[],
      necesidadOtra: v.necesidadOtra,
      comportamiento: v.comportamiento,
      notaTransportista: v.notaTransportista,
      preferencias: v.preferencias as PreferenciaTransporte[],
    });
  }
}
