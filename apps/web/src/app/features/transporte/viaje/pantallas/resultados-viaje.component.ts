...
import {
  ...
  PREFERENCIA_TRANSPORTE_LABELS, PreferenciaTransporte, ResultadoTransporte, resumenSolicitudTransporte,
} from 'shared';
...
import { ICONO_INCLUIDO, OPCIONES_ORDEN, OPCIONES_PREFERENCIAS } from '../transporte-viaje.opciones';
...
export class ResultadosViajeComponent implements OnInit {
  ...
  readonly opcionesIncluidos = OPCIONES_PREFERENCIAS;
  ...
  readonly filtrados = computed(() => {
    ...
    return (this.respuesta()?.resultados ?? []).filter((r) =>
      ...
      && incluidos.every((i) => r.incluidos.includes(i as PreferenciaTransporte)));
  });
  ...
  constructor() {
    ...
    this.filtroIncluidos.valueChanges.pipe(takeUntilDestroyed()).subscribe((lista) => {
      ...
      this.store.actualizar({ filtroIncluidos: lista as PreferenciaTransporte[] });
    });
  }
  ...
  etiquetaIncluido(incluido: PreferenciaTransporte): string {
    return PREFERENCIA_TRANSPORTE_LABELS[incluido] ?? incluido;
  }
