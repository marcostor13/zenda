...
import {
  FranjaTransporte, ModoHorarioTransporte, PatronRecurrenciaTransporte, PuntoViaje, TipoServicioTransporte,
  VueltaTransporte,
...
export class BusquedaViajeComponent implements OnInit {
  ...
  readonly esIdaVuelta = computed(() => this.borrador().tipoServicio === TipoServicioTransporte.IDA_VUELTA);
  readonly esRecurrente = computed(() => this.borrador().tipoServicio === TipoServicioTransporte.RECURRENTE);
  ...
  constructor() {
    ...
    this.form.controls.tipo.valueChanges.pipe(takeUntilDestroyed()).subscribe((tipo) => {
      this.store.elegirTipo(tipo as TipoServicioTransporte);
