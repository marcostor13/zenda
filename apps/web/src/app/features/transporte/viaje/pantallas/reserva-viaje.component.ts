...
import {
  ConfirmacionEntrega, EstadoPresupuesto, PatronRecurrenciaTransporte, PersonaContactoViaje, PresupuestoDto,
  ResultadoTransporte, SolicitudViaje, VerticalKey, instanteDeRecogida, resumenSolicitudViaje,
} from 'shared';
...
import { CrearReservaPayload, ReservaApi, ReservasService } from '../../../reservas/services/reservas.service';
...
export class ReservaViajeComponent implements OnInit, OnDestroy {
  ...
  private readonly presupuesto = signal<PresupuestoDto | null>(null);
  ...
  readonly resumen = computed(() => {
    ...
    return solicitud ? resumenSolicitudViaje(solicitud) : [];
  });
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      if (presupuestoId) await this.prepararPresupuesto(presupuestoId);
      await this.cotizar();
    } catch (error) {
  ...
  /**
   * Viene de «Mis presupuestos»: el viaje sale de la propia petición y el
   * precio, de la oferta de la empresa. La oferta se acepta al pasar al pago.
   */
  private async prepararPresupuesto(presupuestoId: string): Promise<void> {
    const presupuesto = (await this.api.misPresupuestos()).find((p) => p.id === presupuestoId);
    const solicitud = presupuesto?.solicitud['solicitud'] as SolicitudViaje | undefined;
    if (!presupuesto?.importe || !solicitud) throw new Error('Presupuesto no disponible');
    this.presupuesto.set(presupuesto);
    this.store.cargarSolicitud(solicitud, presupuestoId, {
      servicioId: presupuesto.servicioId,
      comercioId: presupuesto.comercioId,
      titulo: presupuesto.tituloServicio ?? '',
      modalidad: solicitud.modalidad,
      total: presupuesto.importe,
    });
    this.importePresupuesto.set(presupuesto.importe);
  }
  ...
  private async cotizar(): Promise<void> {
    ...
    // Con presupuesto el precio ya está pactado: la cotización sólo aporta la
    // política de cancelación y lo incluido, y puede no tener precio cerrado.
    if (!resultado && !this.presupuesto()) {
      throw new Error(respuesta.motivo ?? 'Este transportista ya no puede hacer el viaje.');
    }
    this.resultado.set(resultado ?? null);
    this.prefillYo();
  }
  ...
  private async prepararPago(): Promise<void> {
    ...
    this.errorPago.set(null);
    try {
      const reserva = this.presupuesto() ? await this.reservaDelPresupuesto() : await this.reservas.crear(this.payload());
  ...
  /**
   * Acepta la oferta con los datos de entrega: el API crea la reserva con el
   * importe pactado. Si ya estaba aceptada (el cliente volvió a por el pago),
   * se retoma esa misma reserva.
   */
  private async reservaDelPresupuesto(): Promise<ReservaApi> {
    const presupuesto = this.presupuesto();
    if (!presupuesto) throw new Error('Presupuesto no disponible');
    const aceptado = presupuesto.estado === EstadoPresupuesto.ACEPTADO && presupuesto.reservaId
      ? presupuesto
      : await this.api.aceptarPresupuesto(presupuesto.id, this.detalleDeEntrega());
    if (!aceptado.reservaId) throw new Error('La reserva del presupuesto no se ha creado.');
    return this.reservas.obtener(aceptado.reservaId);
  }

  private detalleDeEntrega(): Record<string, unknown> {
    const b = this.store.borrador();
    return { entrega: { recogida: b.recogida, entrega: b.entrega, confirmacionEntrega: b.confirmacionEntrega } };
  }
