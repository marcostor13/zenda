...
/** Redes en las que Doogking tiene perfil público. */
export type RedSocialKey = 'instagram' | 'facebook' | 'tiktok' | 'x' | 'youtube' | 'linkedin';

/**
 * Marca oficial de una red social (TCK-8008): en el footer y en la landing el
 * usuario debe reconocer el logo, no leer el nombre escrito.
 *
 * Va aparte de `rs-icon` a propósito: los logos de marca son glifos rellenos con
 * geometría fija que no se puede redibujar al estilo de trazo de Lucide sin
 * dejar de ser el logo oficial. `rs-icon` mantiene su contrato (`fill=none`,
 * `stroke=currentColor`) y aquí usamos `fill="currentColor"`.
 */
@Component({
  selector: 'rs-social-icon',
  standalone: true,
...
export class RsSocialIconComponent {
  @Input({ required: true }) name!: RedSocialKey;
  @Input() size: number | string = 20;

  /** Nombre legible para lectores de pantalla, ya que el logo sustituye al texto. */
  @Input() etiqueta = '';
}
