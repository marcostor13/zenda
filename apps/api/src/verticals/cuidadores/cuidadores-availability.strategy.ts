...
/**
 * Vertical Cuidadores a domicilio: visitas al domicilio del cliente, con cupos
 * diarios. Reutiliza el patrón de cupos ya probado en adiestramiento; el precio
 * depende de la modalidad pedida (visita, día completo o noche).
 */
@Injectable()
export class CuidadoresAvailabilityStrategy implements AvailabilityStrategy {
  ...
  async checkAvailability(servicioId: string, params: AvailabilityQuery): Promise<AvailabilityResult> {
    ...
    if (!cuidador) {
      throw new DomainException('Servicio de cuidado a domicilio no encontrado', 404);
    }
    ...
    const tamano = params.parametrosExtra?.['tamano'] as TamanoPerro | undefined;
    // Lista vacía = sin restricción de tamaño.
    if (tamano && cuidador.tamanosAdmitidos?.length && !cuidador.tamanosAdmitidos.includes(tamano)) {
      return { disponible: false, metadata: { motivo: 'tamano_no_admitido' } };
    }

    if (params.parametrosExtra?.['esPPP'] === true && !cuidador.aceptaPPP) {
  ...
  async reserveSlot(servicioId: string, params: ReserveParams): Promise<SlotHold> {
    ...
    if (!disponibilidad.disponible) {
      throw new DomainException('El cuidador no tiene disponibilidad para esa fecha', 409);
    }
