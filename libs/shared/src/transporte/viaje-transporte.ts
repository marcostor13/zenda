 ...
 */
export function horaDeRecogida(solicitud: Pick<SolicitudTransporte, 'modoHorario' | 'hora' | 'franja'>): string | null {
...
export function instanteDeRecogida(
  solicitud: Pick<SolicitudTransporte, 'fecha' | 'modoHorario' | 'hora' | 'franja'>,
  ahora: Date = new Date(),
): Date {
...
export function viajesDeLaSolicitud(solicitud: SolicitudTransporte): number {
  if (solicitud.tipoServicio !== TipoServicioTransporte.RECURRENTE || !solicitud.recurrencia) return 1;
